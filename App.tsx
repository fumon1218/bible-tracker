...
const App: React.FC = () => {
  ...
  const goalStats = useMemo(() => {
    if (!activeMember.startDate || !activeMember.endDate) return null;

    const start = new Date(activeMember.startDate);
    const end = new Date(activeMember.endDate);
            ...
            <div className="flex flex-col">
              ...
              <div className="flex items-center gap-2 mt-1 px-1">
                ...
                <span className="text-[10px] font-bold opacity-80">
                  {connectionStatus === 'connected' ? '실시간 동기화 중 (v2.6 11:38)' : connectionStatus === 'error' ? '연결 끊김 (오류)' : '연결 중...'}
                </span>
              </div>
            </div>
